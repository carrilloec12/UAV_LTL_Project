...
sys.atomic_propositions.add_from({'home', 'goal', 'sX2', 'sX4', 'sX6', 'sX1', 'sX5'})
...
sys.states.add('X1', ap={'sX1'})
sys.states.add('X5', ap={'sX5'})
# UAV 2
sys2 = transys.FTS()
sys2.owner = 'sys'
...
sys2.states.initial.add('X3')    # start in state X6
...
# Add atomic propositions to the states
sys2.atomic_propositions.add_from({'goal', 'home', 's2X2', 's2X4', 's2X6', 's2X1', 's2X5'})
sys2.states.add('X3', ap={'home'})
sys2.states.add('X8', ap={'goal'})
sys2.states.add('X2', ap={'s2X2'})
sys2.states.add('X4', ap={'s2X4'})
sys2.states.add('X6', ap={'s2X6'})
sys2.states.add('X1', ap={'s2X1'})
sys2.states.add('X5', ap={'s2X5'})
...
sys_init = set()
sys_prog = {'home','goal'}
sys_safe = {'((obsX2) -> X (!sX2))', '((obsX2) -> X (!s2X2))',
			'((obsX4) -> X (!sX4))', '((obsX4) -> X (!s2X4))',
			'((obsX6) -> X (!sX6))', '((obsX6) -> X (!s2X6))',
			'!(s2X1 && sX1)',
			'!(s2X5 && sX5)'}
sys_prog |= {'goal'}
...
tomatlab.export('strategy_sim.mat',ctrl)
